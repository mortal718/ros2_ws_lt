...
class JoyTeleopNode(Node):
    ...
    def __init__(self):
        super().__init__('joystick_teleop_mecanum')
    ...
    def joy_callback(self, msg: Joy):
        ...
        if self.activate_button_pressed and not self.button_cross_on and self.button_circle_on:
            self.twist.linear.x = 0.2 * msg.axes[self.axis_linear_id]
            self.twist.linear.y = 0.2 * msg.axes[self.axis_side_id] if self.use_side_vel else 0.0
            self.twist.angular.z = 0.4 * msg.axes[self.axis_angular_id]
