...
: Node("dual_laser_merger", options)
{
  ...
  merged_scan_pub =
    this->create_publisher<sensor_msgs::msg::LaserScan>(this->get_parameter(
      "merged_scan_topic").as_string(), qos_profile);
  merged_cloud_pub =
    this->create_publisher<sensor_msgs::msg::PointCloud>(this->get_parameter(
      "merged_cloud_topic").as_string(), qos_profile);
...
void MergerNode::declare_param()
{
  ...
  enable_calibration_param = this->declare_parameter("enable_calibration", true);
  ...
  const sensor_msgs::msg::LaserScan::ConstSharedPtr & lidar_2_msg)
{
  if (target_frame_param.empty()) {
    ...
    sensor_msgs::msg::PointCloud2 cloud_out_temp;
    sensor_msgs::msg::PointCloud cloud_out;
    pcl::toROSMsg(pcl_cloud_out, cloud_out_temp);
    sensor_msgs::convertPointCloud2ToPointCloud(cloud_out_temp, cloud_out);
    merged_cloud_pub->publish(cloud_out);
    
    merged.header = cloud_out_temp.header;
    ...
    for (sensor_msgs::PointCloud2ConstIterator<float> iter_x(cloud_out_temp, "x"),
      iter_y(cloud_out_temp, "y"), iter_z(cloud_out_temp, "z");
